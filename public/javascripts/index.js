...
define("handleSidebarToggler", [], function () {
  return function () {
    ...
    body.on("click", ".sidebar-toggler", function (e) {
      ...
      $("#sidebar-area .dropdown-collapse")
        .parent("li")
        .children("ul")
        .css("height", "");
...
require(["jquery", "bootstrap", "isIE", "handleSidebarToggler"], function (
  $,
  bootstrap,
  isIE,
  ...
  $("a[name='loadTabContent']").click(function (e) {
    ...
    var tabId = $.escapeSelector($(this).attr("data-id"));
    ...
    $("#" + tabId).trigger("click");
    $("#pageTabContent-" + tabId).html(
      '<object style="width: 100%; height: 800px;" data="' +
        tabSrc +
        '"></object>'
    );
  });
  ...
  $("#sidebar-area .dropdown-collapse").on(
    jQuery.support.touch ? "tap" : "click",
    function (e) {
      e.preventDefault();
  ...
  if (
    ...
    $("body").addClass("sidebar-closed");
    $("#sidebar .nav-second-level, #sidebar .nav-third-level").removeClass(
      "collapse"
    );
  }
...
require(["jquery", "bootstrap", "datatables"], function ($) {
  ...
  $(".toggleActivation").click(function () {
    var self = $(this);
    var id = self.data("id");
    var url = "/admin/users/" + id + "/toggleActivation";
    $.ajax({
      url,
      type: "GET",
      success: function (result) {
        var childEle = self.children("i");
        if (result) {
          childEle.removeClass("fa-times");
          childEle.addClass("fa-check");
        } else {
          childEle.removeClass("fa-check");
          childEle.addClass("fa-times");
        }
      },
    });
  });
